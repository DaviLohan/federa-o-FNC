...
class PenaltySerializer(serializers.ModelSerializer):
    ...
    suspension_remaining = serializers.SerializerMethodField()
    ...
    def get_target_name(self, obj):
        """Retorna nome do alvo (jogador ou time)."""
        if obj.target_type == 'PLAYER' and obj.player:
            return obj.player.player_name or obj.player.user.get_full_name()
        elif obj.target_type == 'TEAM' and obj.team:
            return obj.team.name
        return 'N/A'
    
    def get_suspension_remaining(self, obj):
        """Retorna jogos restantes de suspensão."""
        if obj.games_suspended > 0:
            return max(0, obj.games_suspended - obj.games_served)
        return 0
    ...
    def validate(self, attrs):
        ...
        # Validar que apenas um alvo está definido
        if target_type == 'PLAYER' and not player:
            raise serializers.ValidationError({
                'player': 'Jogador deve ser especificado para penalidade de jogador.'
            })
        if target_type == 'TEAM' and not team:
            raise serializers.ValidationError({
                'team': 'Time deve ser especificado para penalidade de time.'
            })
        if player and team:
            raise serializers.ValidationError(
                'Penalidade deve ser para jogador OU time, não ambos.'
            )
...
class PenaltyAppealSerializer(serializers.ModelSerializer):
    """
    Serializer para recursos de penalidades.
    """
    ...
    def get_penalty_details(self, obj):
        """Retorna detalhes da penalidade."""
        penalty = obj.penalty
        return {
            'id': penalty.id,
            'type': penalty.get_penalty_type_display(),
            'reason': penalty.reason,
            'games_suspended': penalty.games_suspended,
            'target': penalty.player.player_name if penalty.player else penalty.team.name
        }
    
    def create(self, validated_data):
        """Ao criar, definir quem submeteu."""
        request = self.context.get('request')
        if request and hasattr(request, 'user'):
...
class PenaltyAppealReviewSerializer(serializers.Serializer):
    """
    Serializer para revisar recurso de penalidade.
    """
    ...
    def validate(self, attrs):
        ...
        if not decision:
            raise serializers.ValidationError({
                'decision': 'Decisão é obrigatória.'
            })
        return attrs


class SuspensionCheckSerializer(serializers.Serializer):
    """
    Serializer para verificar se jogador/time está suspenso.
    """
    player_id = serializers.IntegerField(required=False)
    team_id = serializers.IntegerField(required=False)
    championship_id = serializers.IntegerField(required=True)
    
    def validate(self, attrs):
        """Validações."""
        if not attrs.get('player_id') and not attrs.get('team_id'):
            raise serializers.ValidationError(
                'Deve especificar player_id ou team_id.'
            )
        if attrs.get('player_id') and attrs.get('team_id'):
            raise serializers.ValidationError(
