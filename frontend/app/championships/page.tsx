...
import { Button, Card, Input, DatePickerInput, Badge, Table, Select, useToast, Skeleton, EmptyState, ImageUpload, PageHeader, FilterBar, SkeletonGrid } from '@/components/shared/ui';
...
export default function ChampionshipsPage() {
  ...
  const createMutation = useMutation({
    ...
    onError: (error: any) => {
      showToast(getApiErrorMessage(error, 'Erro ao criar campeonato'), 'error');
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (error: any) => {
      showToast(getApiErrorMessage(error, 'Erro ao atualizar campeonato'), 'error');
    },
  });
...
function ChampionshipModal({ championship, onClose, onSubmit, isLoading }: ChampionshipModalProps) {
  const [formData, setFormData] = useState({
    ...
    game_end_time: championship?.game_end_time || '',
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (bannerFile || logoFile) {
      ...
      // Campos de agenda de jogos
      if (formData.game_days.length > 0) {
        formDataObj.append('game_days', JSON.stringify(formData.game_days));
      }
      if (formData.game_start_time) {
        formDataObj.append('game_start_time', formData.game_start_time);
      }
      if (formData.game_end_time) {
        formDataObj.append('game_end_time', formData.game_end_time);
      }
      ...
      // Add files — o terceiro argumento garante Content-Disposition: filename="..."
      // sem ele Django recebe o campo sem nome de arquivo e rejeita como não-arquivo
      if (bannerFile) {
        formDataObj.append('banner', bannerFile, bannerFile.name || 'banner.jpg');
      }
      if (logoFile) {
        formDataObj.append('logo', logoFile, logoFile.name || 'logo.jpg');
      }
      ...
      const submitData: any = {
        ...
        game_start_time: formData.game_start_time || null,
        game_end_time: formData.game_end_time || null,
      };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4 overflow-y-auto py-8">
      <Card className="max-w-3xl w-full my-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Agenda de Jogos */}
          <div className="space-y-4 p-4 bg-surface2/50 rounded-xl border border-gold/30">
            <h4 className="text-sm font-bold text-gold mb-3">Agenda de Jogos</h4>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  const isSelected = formData.game_days.includes(code);
                  return (
                    <button
                      key={code}
                      type="button"
                      onClick={() => {
                        setFormData((prev) => ({
                          ...
                          game_days: isSelected
                            ? prev.game_days.filter((d: string) => d !== code)
                            : [...prev.game_days, code],
                        }));
                      }}
                      className={`
                        ...
                        ${isSelected
                          ...
                          : 'bg-surface2 border-border text-muted hover:border-gold/50 hover:text-text'
                        }
                      `}
                    >
                      {label}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormData((prev) => ({ ...prev, game_start_time: e.target.value }))}
                  className="w-full px-4 py-3 bg-surface2 border border-border rounded-xl text-text focus:outline-none focus:ring-2 focus:ring-gold/50 focus:border-gold transition-all"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-text mb-2">
                ...
                <input
                  ...
                  onChange={(e) => setFormData((prev) => ({ ...prev, game_end_time: e.target.value }))}
                  className="w-full px-4 py-3 bg-surface2 border border-border rounded-xl text-text focus:outline-none focus:ring-2 focus:ring-gold/50 focus:border-gold transition-all"
                />
              </div>
            </div>
            ...
            {/* Preview da agenda */}
            {formData.game_days.length > 0 && formData.game_start_time && formData.game_end_time && (
              <div className="p-3 bg-gold/10 rounded-xl border border-gold/30">
                <p className="text-sm text-gold font-medium">
                  Jogos: {formData.game_days.map((d: string) => {
                    const dayMap: Record<string, string> = {
                      MON: 'Seg', TUE: 'Ter', WED: 'Qua',
                      THU: 'Qui', FRI: 'Sex', SAT: 'Sab', SUN: 'Dom'
                    };
                    return dayMap[d] || d;
                  }).join(', ')} das {formData.game_start_time} as {formData.game_end_time}
                </p>
              </div>
            )}
          </div>
