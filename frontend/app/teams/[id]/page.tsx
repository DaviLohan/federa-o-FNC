...
import { TacticalBoard } from '@/components/teams/TacticalBoard';
    ...
    return (
      <div>
        <p className="text-sm text-muted mb-6 leading-relaxed">
          Selecione uma partida agendada, escolha a formação e posicione os jogadores no campo.
          A escalação fica salva e os jogadores são notificados automaticamente.
        </p>
        <TacticalBoard team={team} members={members} />
      </div>
    );
  }
  return (
    <div className="flex flex-col items-center justify-center py-16 gap-4 text-muted">
      <div className="w-16 h-16 rounded-2xl bg-gold/5 border border-gold/15 flex items-center justify-center">
        <Crosshair className="w-8 h-8 text-gold/30" />
      </div>
      <div className="text-center space-y-1">
        <p className="text-sm font-medium text-text/60">Escalação definida pelo dono do time</p>
        <p className="text-xs text-muted max-w-xs">
          Consulte a página de cada partida para ver a escalação confirmada.
        </p>
      </div>
      <Button variant="ghost" className="mt-1" onClick={() => (window.location.href = '/matches')}>
        Ver Partidas
      </Button>
    </div>
  );
...
export default function TeamDetailPage() {
  ...
  const members: TeamMembership[] = Array.isArray(membersRaw) ? membersRaw : [];
        ...
        {isOwner && leaveRequests.length > 0 && (
          ...
          >
            ...
            {leaveRequests.map((lr) => (
              <div key={lr.id} className="flex items-center justify-between gap-4 text-sm">
                <div>
                  ...
                  <div className="space-y-6">
                    {isOwner ? (
                      <MemberSubTabs
                        ...
                        onInviteSent={() => queryClient.invalidateQueries({ queryKey: ['team-members', teamId] })}
                      />
                    ) : (
                      <MembersTab
                        teamId={teamId} members={members} isLoading={membersLoading}
                        onMemberRemoved={() => queryClient.invalidateQueries({ queryKey: ['team-members', teamId] })}
                        readOnly
                      />
                    )}
                  </div>
