...
export function TeamsTab({ enrollments, championship }: TeamsTabProps) {
  ...
  const pendingTeams = enrollments.filter((e) => e.status === 'PENDING');
  const otherTeams = enrollments.filter((e) => e.status !== 'APPROVED' && e.status !== 'PENDING');
  ...
  return (
    <div className="space-y-6">
      ...
      {pendingTeams.length > 0 && (
        <div>
          <h2 className="text-xl font-bold text-text mb-4">Aguardando Aprovação</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
